...
let comments: string[] = [];
let comAnswers: string[] = [];
let arrowAnswer: string = '';
let indexArrow: string = '';
let drawAnswer: string = '';

document.getElementById('comment-send').onclick = function(): void {  
    event.preventDefault();
    let commentBody: HTMLElement | null = document.getElementById('comment-body');
...
function localComments(): void {                                  // отображаем из Local
...
function showComments(): void {                                    // рисуем отправленный коммент
    ...
    if(resultComment){    
        ...
        comments.forEach(function(item: { userSend: any; time: number; body: any; like: boolean; ratingScore: any; }, index: number) {
            //её тут объявим
            let out = '';
...
function timeConverter(UNIX_timestamp: number): string {
    ...
    let months: any = ['01','02','03','04','05','06','07','08','09','10','11','12'];
...
function toggleHeartAnswer(index: number): void {    
...
function paintHeart(like: boolean): any {
...
function changeRating(): void {   
    document.querySelectorAll('.rating').forEach(function(item) {
        item.addEventListener("click", function(event) {    
            ...
            const indRat: number = btn.getAttribute('data-index-change');
...
function changeRatingAnswer(index: number) :void {   
    document.querySelectorAll('.rating-answer').forEach(function(item) {
        item.addEventListener("click", function(event) {    
            const btnAns = event.target.closest('.rating-answer');
            const indRatAns: number = btnAns.getAttribute('data-index-change-answer');
...
function submitAnswer(): void {
    document.querySelectorAll('.submit-answer').forEach(function(item){  
        item.addEventListener('click', function(subm){
            subm.preventDefault();
            let answertBody: HTMLElement | null = document.getElementById('idAnswer');
...
function answerContentDraw(index: number): void {
...
function filterInFavorite(): void {
...
function saveCommentsFavorite(): void {     
    let commFav = comments.filter((item: { like: boolean; }) => item.like  === true)
...
function saveCommentRating(): void {      // сортируем и сохраняем рейтинг в Local 
    let reverseRating: Element | null = document.querySelector('.svg-arrow');
    let commRat = comments.sort((a: number, b: number) => a.ratingScore > b.ratingScore ? 1 : -1);
...
function localCommentsRating(): void {     // отображаем отсортированный рейтинг из Local                                
...
function localCommentsRelevance(): void {     // отображаем отсортированный по актуальности из Local                                
...
function saveCommentsAnswer(): void {      // сортируем и сохраняем по колличеству ответов в Local 
    let reverseRating = document.querySelector('.svg-arrow');
    let commAns = comments.sort((a: { answer: number; }, b: { answer: number; }) => a.answer > b.answer ? 1 : -1)
...
function localCommentsAnswer(): void {     // отображаем отсортированный по колличеству ответов из Local                                
...
function filterSelect(textSelect: string): void{
